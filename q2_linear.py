...
class Linear(DQN):
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        q2 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=target_q_scope)
        op = [tf.assign(q2[i], q[i]) for i in range(len(q))]
    ...
    def add_loss_op(self, q, target_q):
        ...
        q_sa = tf.reduce_sum(tf.multiply(action,q),axis = 1)
        
        self.loss = tf.reduce_mean(tf.squared_difference(q_sa, q_samp))
    ...
    def add_optimizer_op(self, scope):
        ...
        scope_var = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=scope)
#        grads, var = list(zip(*optimizer.compute_gradients(self.loss, scope_var)))
#        
#        if self.config.grad_clip:
#           grads = tf.clip_by_norm(grads, self.config.clip_val)
#        self.train_op = optimizer.apply_gradients(zip(grads, var))
#        self.grad_norm = tf.global_norm(grads)

        grads_var = optimizer.compute_gradients(self.loss, scope_var)
        if self.config.grad_clip:
           clipped = [(tf.clip_by_norm(t[0],self.config.clip_val),t[1]) for t in grads_var] 
        self.train_op = optimizer.apply_gradients(clipped)
        self.grad_norm = tf.global_norm([t[0] for t in grads_var])
